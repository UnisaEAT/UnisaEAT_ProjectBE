...
exports.insertFAQ = (req, res) => {
  ...
  if (domanda.length <= 17 || domanda.length > 200) {
    res.json
  ...
  if (risposta.length <= 17 || risposta.length > 200) {
    res.json
...
exports.updateFAQ = (req, res) => {
  ...
  const domanda = req.body.domanda
  const newdomanda = req.body.newdomanda
  const newrisposta = req.body.newrisposta

  // validazione domanda

  if (domanda.length != 0) {
    if (newdomanda.length <= 17 || newdomanda.length > 200) {
      res.json
  ...
  if (newrisposta.length != 0) {
    if (newrisposta.length <= 17 || newrisposta.length > 200) {
      res.json
